...
function createTransitionFluxReport( simDB::SQLite.DB, idKey::String,
    transDBname::String, timeGrid::Vector{Float64}, transition::TransitionType )
    queryPartCmd = string( "SELECT count( `", idKey,
        "` ) counts FROM `", transDBname, "` WHERE\n    ",
    ...
    for ii in eachindex( timeGrid )
        ...
        result[ii] = DataFrame( DBInterface.execute( simDB, queryCmd ) )[1, 1]
    end  # for ii in eachindex( timeGrid )

    return result
end  # createTransitionFluxReport( simDB, idKey, transDBname, timeGrid,
     #    transition )

createTransitionFluxReport( mpSim::MPsim, timeGrid::Vector{Float64},
    transition::TransitionType ) = createTransitionFluxReport( mpSim.simDB,
    mpSim.idKey, mpSim.transDBname, timeGrid, transition )
...
generateTransitionQuery( transition::String ) =
    string( "transition IS '", transition, "'" )
    # string( "transition IS '", transition, "' AND",
    # "\n    sourceNode IS NOT 'active' AND",
    # "\n    targetNode IS NOT 'active'" )
...
function generateTransitionQuery( transition::NTuple{2,String} )
    ...
    targetNode = lowercase( transition[2] ) ∈ specialNodes ? "NULL" :
        string( "'", transition[2], "'" )
    return string( "sourceNode IS ", sourceNode, " AND",
        "\n    targetNode IS ", targetNode )
